  ...
  methods: {
        ...
        res.forEach((item) => {
          ...
          menuRouterLists.push({
            ...
            meta: {
              ...
              // activeMenu: `/${route.menuGrpName}_${item.itemName}`
            },
            children: [
              // {
              //   // path: `tree/:id(\\d+)`,
    ...
    getChildrenMenu(route, key, role) {
      ...
      getMenuLvAfter({
        ...
        // })
        this.recursionRotes2(this.routesArray, menuRouterLists, key)
        console.log('点击侧边栏 新路由表:', this.routesArray)
        this.$store.commit('user/SET_ROUTERS', JSON.parse(JSON.stringify(this.routesArray)))
    ...
    recursionRotes2(routes, child, key) {
      routes.forEach((item) => {
        if (item.path === key) {
          item['children'] = child
        }
        if (item.children && item.children.length > 0) {
          this.recursionRotes2(item.children, child, key)
        }
      })
    },
